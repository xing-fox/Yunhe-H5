        ...
        <div class="shop-activity-section shop-activity-interact">
            ...
            <div class="shop-activity-interact" v-if="content.note || content.comment">
                <div class="shop-activity-arrow-title shop-activity-borderline">
                    ...
                    <i class="arrow fr" onclick="shop.goLink(this,1)" data-href="shop_activity_interaction.html">全部&gt;</i>
                </div>
                <template v-if="content.note && content.note.length>0">
                    <noteItem :dataItem="iem" v-on:zanClick="zanClick(idex,'note')" :key="idex" v-for="( iem, idex ) in content.note"></noteItem>
                </template>
                <template v-if="content.comment && content.comment.length>0">
                    <commentItem :dataItem="item" v-on:zanClick="zanClick(index,'comment')" :key="index" v-for="( item, index ) in content.comment"></commentItem>
                </template>
            </div>
  ...
  methods: {
      ...
      if (type === 'note') {
        ...
        that.$http.like({
          ...
          if (res.success && res.code === 'E00000') {
            let flag = parseInt(that.content[type][index][type + '_like_flag'])
            that.content[type][index][type + '_like_flag'] = -flag
            that.content[type][index][type + '_like_total'] = parseInt(that.content[type][index][type + '_like_total']) + flag * 1
          }
        })
      } else {
        /* 评论点赞 */
        that.$http.commentlike({
          openid: window.localStorage.getItem('openId') || window.sessionStorage.getItem('openId'),
          data: JSON.stringify({
            'focus_type': 21,
    ...
    writeSubmit (data) {
      ...
      if (data[0]) {
        let that = this
        /* 刷新评论数据 */
        that.$http.ShopActionDetailNoteComment({
          openid: window.localStorage.getItem('openId') || window.sessionStorage.getItem('openId'),
          data: JSON.stringify({
            'note_pag_no': 1,
            'note_pag_num': 5,
      ...
      if (this.content.action.action_join_flag === -1) {
        ...
        that.$http.joinShopAction({
          ...
          if (res.success && res.code === 'E00000') {
            ...
            that.content.action.join_total_f = parseInt(that.content.action.join_total_f) + 1
...
<style>
    ...
    .shop-activity-banner{
        ...
        position: relative;
        min-height: 1.8750rem;
    }
    .shop-activity-banner .video-mask-img,.shop-activity-banner .video-ico-play{
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
    ...
    .shop-activity-abstract-right .icon-ico_success.active{
        color: #42A157;
        animation: join .5s ease both;
        -webkit-animation: join .5s ease both;
    }
    .shop-activity-title{
        font-size: .16rem;
        color: #222;
    ...
    .shop-activity-coupon{
        ...
        background-image: image-set(url(./img/coupon@1.5x.png) 1.5x, url(./img/coupon@2x.png) 2x, url(./img/coupon@3x.png) 3x);
        background-image: -webkit-image-set(url(./img/coupon@1.5x.png) 1.5x, url(./img/coupon@2x.png) 2x, url(./img/coupon@3x.png) 3x);
    ...
    .shop-activity-store-info-addr,.shop-activity-store-info-time{
        font-size: .14rem;
        color: #666;
        line-height: 1.8;
    }
    .shop-activity-store-info-addr i:before,.shop-activity-store-info-time i:before{
       margin-right: .05rem;
    }
