<template>
    <div class="interact-list-item shop-activity-borderline">
        <div class="interact-list-item-hd">
            <i class="user-pic" :style="{backgroundImage: 'url('+dataItem.customer_picture+')'}">
                <svg class="icon icon-jiav" aria-hidden="true">
                    <use xlink:href="#icon-jiav"></use>
                </svg>
            </i>{{ dataItem.customer_name }}
        </div>
        <div class="interact-list-item-bd">
            <h4 v-if="dataItem.note_name">{{ baseChange(dataItem.note_name) }}</h4>
            <p>{{ baseChange(dataItem.note_desc) }}</p>
        </div>
...
export default {
  ...
  computed: {
    /* 实现某一属性的实时计算 */
    numChange () {
      let num = this.dataItem.note_like_total
      num = parseInt(num)
